...
static char *pgss_smem_buffer = NULL;
static pgssSharedMem *pgss_smem = NULL;
...
static const struct config_enum_entry pgss_transient_options[] = 
{
  {"file", PGSS_TRANSIENT_FILE, false},
  {"shared_memory", PGSS_SHARED_MEMORY, false}
};
...
static int pgss_transient_type;			/* whether to share stats using file or memory */
static int pgss_smem_size;			/* whether to share stats using file or memory */
...
_PG_init(void)
{
	...
	DefineCustomEnumVariable("pg_stat_statements.transient_type",
							 "Transient media for pg_stat_statements statistics across server.",
							 NULL,
							 &pgss_transient_type,
							 PGSS_TRANSIENT_FILE,
               pgss_transient_options,
							 PGC_SUSET,
							 0,
							 NULL,
							 NULL,
	...
	DefineCustomIntVariable("pg_stat_statements.shared_memory_size",
							 "Shared memory size for pg_stat_statements statistics.",
							NULL,
							&pgss_smem_size,
							1048576, /* Default shared memory size is 1GB */
							1024,
							INT_MAX,
							PGC_POSTMASTER,
							0,
	...
	RequestNamedLWLockTranche("pg_stat_statements_shared_memory_config", 1);
...
pgss_shmem_startup(void)
{
	...
	pgss = NULL;
	pgss_hash = NULL;
  pgss_smem_buffer = NULL;
	
  if (pgss_transient_type == PGSS_SHARED_MEMORY)
  {
    pgss_smem = ShmemInitStruct("pg_stat_statements_shared_memory_config",
		  				               sizeof(pgss_smem),
			  			               &found);
    if (!found)
    {
		  /* First time through ... */
		  pgss_smem->lock = &(GetNamedLWLockTranche("pg_stat_statements_shared_memory_config"))->lock;
      pgss_smem->head = 0;
      pgss_smem->tail = 0;
    }
  
    pgss_smem_buffer = ShmemInitStruct("pg_stat_statements_shared_memory",
		  				               pgss_smem_size,
			  			               &found);
    if (!found)
    {
		  /* First time through ... */
      pgss_smem_buffer[0] = '\0';
      pgss_smem->head = 0;
      pgss_smem->tail = 0;
    }
  } 
  ...
  if (pgss_transient_type != PGSS_TRANSIENT_FILE)
    return;
			...
			Size *query_offset, int *gc_count)
{
  ...
  else
  {
    ...
    if (!found)
    {
	  ...
	  pgss_smem_buffer = ShmemInitStruct("pg_stat_statements_shared_memory", pgss_smem_size, &found);
    if (!found)
    {
		  /* First time through ... */
      pgss_smem_buffer[0] = '\0';
    ...
    /* Acquire a lock to write to the shared buffer. */
	  LWLockAcquire(pgss_smem->lock, LW_EXCLUSIVE);
 
    elog(WARNING, "%d, %d, %d", pgss_smem->head , query_len , pgss_smem_size);

    if ((pgss_smem->head + query_len) < pgss_smem_size)
    {
      /* Add query text into the shared buffer */
      memcpy(pgss_smem_buffer + pgss_smem->head , query , query_len);
      pgss_smem->head += query_len;
    
      /* Append the null termination marker to the buffer */
      *(pgss_smem_buffer + pgss_smem->head) =  '\0';
      pgss_smem->head++;

	    LWLockRelease(pgss_smem->lock);
    }
    else
	    ereport(WARNING,
			  (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			   errmsg("no space left in shared memory")));
  }
...
qtext_load_file(Size *buffer_size)
{
  ...
  else
  {
    ...
    buf = (char *) malloc(pgss_smem->head);
    memcpy(buf, pgss_smem_buffer, pgss_smem->head);
    *buffer_size = pgss_smem->head;
